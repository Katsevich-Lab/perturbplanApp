...
#' @importFrom shiny conditionalPanel renderUI uiOutput
...
#' @importFrom shiny showNotification downloadHandler renderPlot
...
mod_results_display_server <- function(id, plot_objects, analysis_results) {
  moduleServer(id, function(input, output, session) {
      ...
      tagList(
        ...
        tags$div(
          ...
          if (plots$plot_type == "single_parameter_curve") {
            tagList(
              tags$strong("Designs Evaluated: "), 
              tags$span(summary_data$total_designs_evaluated),
              tags$br(),
              tags$strong("Feasible Designs: "), 
              tags$span(summary_data$feasible_designs),
              tags$br(),
              tags$strong("Success Rate: "), 
              tags$span(paste0(round(summary_data$feasibility_rate * 100, 1), "%"))
            )
          } else {
            tagList(
              tags$strong("Total Combinations: "), 
              tags$span(summary_data$total_designs_evaluated),
              tags$br(),
              tags$strong("Power Feasible: "), 
              tags$span(summary_data$power_feasible_designs),
              tags$br(),
              tags$strong("Budget Feasible: "), 
              tags$span(summary_data$budget_feasible_designs)
            )
          }
        ),
        ...
        # Recommendation
        tags$div(
          style = "background-color: #E8F4FD; padding: 10px; border-radius: 5px; border-left: 4px solid #2E86AB;",
          h5("Recommendation", style = "color: #2E4A62; margin-bottom: 8px;"),
          tags$p(
    ...
    observeEvent(input$export_excel, {
      ...
      tryCatch({
        ...
        excel_data <- list(
          ...
          "Configuration" = create_excel_config(results$user_config),
          "Metadata" = data.frame(
        ...
        # Generate filename
        filename <- paste0("perturbplan_analysis_", 
                          format(Sys.time(), "%Y%m%d_%H%M%S"), 
                          ".xlsx")
        
        # Write Excel file
        write.xlsx(excel_data, file = filename)
        
        showNotification(
          paste("Results exported to", filename),
          type = "success",
          duration = 5
        )
    ...
    # Plot download (placeholder - would need proper implementation)
    observeEvent(input$export_plot, {
      showNotification(
        "Plot download functionality to be implemented",
        type = "message",
        duration = 3
      )
    })
...
#' @return Data frame for Excel export
#' @noRd
create_excel_config <- function(config) {
  ...
  data.frame(
    ...
    Value = c(
      design$optimization_type,
      paste0(design$target_power * 100, "%"),
      if (!is.null(design$cost_budget)) paste0("$", scales::comma(design$cost_budget)) else "None",
      design$minimization_target
    )
  )
}
