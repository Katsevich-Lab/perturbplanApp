...
perform_cost_minimization_analysis <- function(config, pilot_data) {
  ...
  # Always ensure minimum_fold_change and TPM_threshold are fixed when optimizing cost
  perturbplan_params$fixed_variable$minimum_fold_change <- config$effect_sizes$minimum_fold_change_fixed
  perturbplan_params$fixed_variable$TPM_threshold <- as.numeric(config$analysis_choices$TPM_threshold_fixed)
