...
get_parameter_ranges <- function(biological_system = "K562", experimental_platform = "10x Chromium v3") {
  ...
  list(
    ...
    sequenced_reads_per_cell = list(min = 500, max = 20000, step = 500, default = 5000),
...
format_parameter_value <- function(value, parameter_type) {
  switch(parameter_type,
    ...
    "sequenced_reads_per_cell" = paste(scales::comma(value), "reads/cell"),
...
validate_parameter_combinations <- function(parameter_grid) {
  ...
  if ("cells_per_target" %in% names(parameter_grid) && 
      "sequenced_reads_per_cell" %in% names(parameter_grid)) {
    total_reads <- parameter_grid$cells_per_target * parameter_grid$sequenced_reads_per_cell
