...
- **Statistical power** is the expected proportion of non-null pairs that are rejected. In CRISPR screens, power represents your ability to confidently identify perturbed elements that truly have an effect on the gene of interest.
...
- **Experimental cost** is the total financial investment required to run your screen. Cost is determined by two main factors:
  ...
  - **Sequencing cost**: Number of cells × sequenced reads per cell × cost per million reads (cost/million reads).
...
- **What's fixed**: Sequenced reads per cell, expression threshold, fold change.

#### **Workflow 2: Minimize Reads per Cell**

- **Goal**: Find the minimum sequencing depth (sequenced reads per cell) needed to achieve target power.
...
- **What's fixed**: Cells per target, sequenced reads per cell, fold change.
...
- **What's fixed**: Cells per target, sequenced reads per cell, expression threshold.

#### **Workflow 5: Minimize Total Cost**

- **Goal**: Find the combination of cells and reads that achieves target power at minimum cost.

- **What varies**: Both cells per target AND sequenced reads per cell (jointly optimized).
...
- **What's fixed**: Sequenced reads per cell, fold change.
- **When to use**: You can adjust cell numbers but sequenced reads per cell is fixed.

##### **Workflow 7: Expression Threshold Minimization (Reads Varying)**
- **What varies**: Expression threshold, sequenced reads per cell.
- **What's fixed**: Cells per target, fold change.
- **When to use**: You can adjust sequenced reads per cell but cells per target is fixed.

##### **Workflow 8: Expression Threshold Minimization (Cells + Reads Varying)**
- **What varies**: Expression threshold, cells per target, sequenced reads per cell.
- **What's fixed**: Fold change only.
- **When to use**: Maximum flexibility - you can adjust both cells per target and sequenced reads per cell.
...
- **What's fixed**: Sequenced reads per cell, expression threshold.
- **When to use**: You want to detect small effects; sequenced reads per cell is constrained.

##### **Workflow 10: Fold Change Minimization (Reads Varying)**
- **What varies**: Fold change, sequenced reads per cell.
- **What's fixed**: Cells per target, expression threshold.
- **When to use**: You want to detect small effects; cells per target is constrained.

##### **Workflow 11: Fold Change Minimization (Cells + Reads Varying)**
- **What varies**: Fold change, cells per target, sequenced reads per cell.
...
- **Reference expression data**: Expression data used to calculate statistical power. This data provides baseline gene expression statistics, library parameters and mapping efficiency that inform power calculations.
  ...
  - **Custom data format**: The input must be an RDS file with the same structure as the output from [reference_data_preprocessing( )](https://katsevich-lab.github.io/perturbplan/reference/reference_data_processing.html) function in the PerturbPlan package. The RDS file must contain:
...
- **Cells per target**: The averaged number of cells receiving gRNAs targeting the same element.

- **Sequenced reads per cell**: The averaged number of reads per cell when sequencing.
...
- **Mapping efficiency**: The ratio of sequencing reads that map confidently to the genes of interest. 
  ...
  - **Why it matters**: Lower mapping efficiency reduces confidently mapped reads, requiring more total raw reads to achieve the same power.
  ...
  - **Typical values**: 
    ...
    - **0.5-0.7**: High-quality TAP-seq libraries.
...
- **FDR target level**: pre-specified level for false discovery rate control. 
